...
CREATE OR REPLACE VIEW remove_general_covid_case as
...
where product.code = 'NCOV'
  ...
  and(
        ...
        OR
            (mview_covid.CASE_INTERVIEW_STATUS IS NOT NULL AND mview_covid.CASE_INTERVIEW_STATUS != '1')
        --Question.CASE_INTERVIEW_STATUS.Value <> NULL AND NOT '1' (LTF)
        OR
...
alter view remove_general_covid_case owner to covidsalesforce_dms_user;
grant select on  remove_general_covid_case  to covidsalesforce_user;